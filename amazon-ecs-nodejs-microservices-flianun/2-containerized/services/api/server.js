...
router.get('/flianun/threads-flianun', function *() {
  this.body = db.threads;
});

router.get('/flianun/threads-flianun/:threadId', function *() {
  const id = parseInt(this.params.threadId);
  this.body = db.threads.find((thread) => thread.id == id);
});

router.get('/flianun/posts-flianun', function *() {
  this.body = db.posts;
});

router.get('/flianun/posts-flianun/in-thread/:threadId', function *() {
  const id = parseInt(this.params.threadId);
  this.body = db.posts.filter((post) => post.thread == id);
});

router.get('/flianun/posts-flianun/by-user/:userId', function *() {
  const id = parseInt(this.params.userId);
